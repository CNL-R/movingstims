...
ERPavg = zeros(size(id,2),2,256,512); %% ERIC!! CHANGE SO ADAPTABLE!!!! MUCH WORK
...
for i = 1:size(id,2)
    ...
    % Avgerage across
    for j = 1:10  %% ERIC, NEEDS BE MADE ADAPTABLE, LOOP FOR TRIGGER VALUES
       ERPavg(i,j,:,:) = mean(ERPs{j}.data,3); %3 is an indexing thing in the data structure
    end
    ERPavg(i,11,:,:)=ERPavg(i,2,:,:)+ERPavg(i,3,:,:); %low vis + low aud
    ERPavg(i,12,:,:)=ERPavg(i,5,:,:)+ERPavg(i,6,:,:); %med vis + med aud
    ERPavg(i,13,:,:)=ERPavg(i,8,:,:)+ERPavg(i,9,:,:); %high vis + high aud
