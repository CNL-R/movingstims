...
%101080014 - Appending THREE Log Files
...
%10108014 Change - Appending 3rd Log File
filename = '10108014_3-IETwo.log';
[struct, cond] = importPresentationLog(strcat(path,filename));
thirdblock = cond.code;                                          %block is a cell array
tempblock = [];                                             %converting block to a double array
for i = 1:size(thirdblock)                                       
    tempblock = [tempblock str2num(cell2mat(thirdblock(i)))];
end 
thirdblock = tempblock;
thirdblock(end) = 0;

block = [block secondblock thirdblock];
block(end) = 0;
